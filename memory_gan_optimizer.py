...
class GenerateChallengingQuestion(dspy.Signature):
    ...
    challenging_question = dspy.OutputField(
        desc="A question designed to be hard for a memory-based system but answerable from the source text."
    )
...
class MemoryGAN(dspy.Module):
    ...
    def forward(self, source_text):
        ...
        pred_reference_answer = self.reference_module(
...
trainset = [
    dspy.Example(
        source_text="The Eiffel Tower, located in Paris, France, was completed in 1889. It was designed by Gustave Eiffel and is one of the most recognizable landmarks in the world. Initially criticized by some of France's leading artists and intellectuals for its design, it has since become a global cultural icon of France and one of the most recognizable structures in the world. The tower is 330 metres (1,083 ft) tall, about the same height as an 81-storey building, and is the tallest structure in Paris. Its base is square, measuring 125 metres (410 ft) on each side."
    ).with_inputs("source_text"),
    dspy.Example(
        source_text="Photosynthesis is a process used by plants, algae, and certain bacteria to convert light energy into chemical energy, through a process that uses sunlight, water, and carbon dioxide. This chemical energy is stored in carbohydrate molecules, such as sugars, which are synthesized from carbon dioxide and water. Oxygen is also released as a byproduct. Most plants, most algae, and cyanobacteria perform photosynthesis; such organisms are called photoautotrophs. Photosynthesis is largely responsible for producing and maintaining the oxygen content of the Earth's atmosphere, and supplies most of the energy necessary for life on Earth."
    ).with_inputs("source_text"),
    dspy.Example(
        source_text="Quantum computing uses quantum-mechanical phenomena like superposition and entanglement to perform computation. Qubits can exist in multiple states simultaneously, enabling quantum computers to solve certain problems much faster than classical computers. However, qubits are fragile and require extremely cold temperatures to maintain their quantum states."
    ).with_inputs("source_text"),
    dspy.Example(
        source_text="The human brain contains approximately 86 billion neurons, each connected to thousands of other neurons through synapses. This complex network enables processes like learning, memory, and consciousness. Neuroplasticity allows the brain to reorganize itself by forming new neural connections throughout life."
    ).with_inputs("source_text"),
]
...
def main():
    ...
    with mlflow.start_run(run_name="SIMBA_MemoryGAN_Run"):
        ...
        print("Optimizing MemoryGAN program with SIMBA...")
        try:
            ...
            optimized_program = simba_optimizer.compile(
                program_to_optimize, trainset=trainset
            )
        ...
        if os.getenv("FIRECRAWL_API_KEY"):
            try:
                ...
                validation_set = [
                    dspy.Example(
                        source_text=get_firecrawl_content(
                            "https://en.wikipedia.org/wiki/List_of_unusual_animals"
                        )
                    ).with_inputs("source_text"),
                    dspy.Example(
                        source_text=get_firecrawl_content(
