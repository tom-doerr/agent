...
class GraphModuleSignature(dspy.Signature):
    ...
    system_context = dspy.InputField(desc="Context: num_modules, max_steps, current_step")
    data = dspy.InputField(desc="Main data being processed")
    notes = dspy.InputField(desc="Additional notes passed between modules")
    next_module = dspy.OutputField(desc="Index of next module to execute (0 to num_modules-1)")
