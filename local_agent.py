...
safety_checker = dspy.Predict('command_run_history, command, user_request -> command_is_safe_to_execute: bool, reasoning', instructions="The commands are generated by an LLM and we trust the LLM to not be malicious. The LLM is working unsupervised. If the command is not obviously dangerous and/or very hard to impossible to undo (like an rm command of files existing before the session started) then it should be marked as safe")
